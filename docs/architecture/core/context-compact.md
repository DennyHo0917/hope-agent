...
3. **完整工具组而不是单条消息是最小协议单位。** 所有模型服务商的最新调用/结果配对和媒体块组成硬保护后缀，其所属的原始用户消息另设逐项保护锚点；定位标识重复或缺失、同轮结构不完整、候选来源校验值不匹配时，一律重新规划或安全拒绝。
...
- 零网络确定性套件 `context-compaction-safety@1.1.0` 直接调用生产算法和数据库事务，验证协议、安全、持久化及失败关闭不变量；它不依赖模型发挥，也不进入普通 `cargo test`。
...
若 C0 在完整请求中仍不可发送，容量恢复状态机固定为 `规划 → 第 0 层 → 规划 → 第 2 层 → 规划 → 第 3 层 → 规划 → 类型化终态`；每次规划都重新计算同一个模型服务商请求，第 0/2 层只改旧历史的请求投影视图，第 3 层才改权威会话历史。最新完整调用/结果组与其所属原始用户消息分别作为硬保护锚点；同一用户回合中更早完成的轮次可先降档，再由第 3 层摘要。摘要写在前，原始用户消息以原生项放回最新组前，注入的运行时台账和文件恢复项位于两者之前；安装后逐项验证两个锚点及调用/结果配对，再以“先持久化、后生效”的检查点发布。检查点失败不会污染智能体内存历史。最终仍不可发送时返回 `CurrentToolGroupOverflow` 结构化终态，禁止盲目换模型或重跑副作用。
