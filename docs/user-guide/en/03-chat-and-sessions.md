...
- **Import Codex history**: click the import button at the top of the session sidebar to read local Codex sessions from `CODEX_HOME` (default `~/.codex`) on the machine running Hope Agent. Repeating the action updates only changed records. Imported records can be searched, renamed, archived, and exported, but remain read-only: you cannot send messages, run tools, or fork them. Tool calls, reasoning, image contents, Codex-injected runtime context, and credentials are excluded. In web mode, the server machine is the source; deleting the Hope copy leaves the Codex files untouched.
