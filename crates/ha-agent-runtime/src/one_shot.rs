//! Feature-owned unified one-shot LLM adapter.
//!
...
use super::provider_adapters::openai_chat_adapter::parse_chat_completions_sse;
use super::provider_adapters::openai_responses_adapter::parse_openai_sse;
#[cfg(test)]
use super::types::CacheSafeParams;
use super::types::{AssistantAgent, ChatUsage, LlmProvider, ProviderFormat};
use ha_core::agent::llm_adapter::{
    LlmApiAdapter, OneShotMode, OneShotRequest, OneShotResult, OneShotRuntime,
};
...
fn as_adapter(provider: &LlmProvider) -> Box<dyn LlmApiAdapter + '_> {
    match provider {
...
#[async_trait]
impl OneShotRuntime for Runtime {
    async fn one_shot(
        &self,
        ...
        as_adapter(provider).one_shot(client, req).await
    }
    ...
    async fn one_shot_stream(
        ...
        as_adapter(provider)
            .one_shot_stream(client, req, cancel, on_delta)
            .await
    }
}
