...
use ha_core::git_control::{
    git_command, output_error, repository_revision, run_git, run_git_bytes, untracked_paths,
};
use ha_core::session::{
    effective_working_dir_for_meta, SessionDB, WorkspaceGitCommit, WorkspaceGitStatus,
    WorkspaceGitSync, WorkspaceGitSyncState,
};
// 壳层（routes/commands）经本模块单一路径取用簿记行类型。
pub use ha_core::git_control::GitOperationRun;
...
pub fn load_control_snapshot(
    ...
    let rich = ha_core::session::build_git_snapshot(&ctx.checkout_root)
...
pub fn switch_branch(
    ...
    with_idempotent_operation(db, session_id, &input.request_id, "switch_branch", |ctx| {
        ...
        match selected.kind {
            ha_core::filesystem::GitBranchKind::Local => {
                run_git_ok(
                    &ctx.workspace_root,
                    &["switch", "--no-guess", &selected.name],
                )?;
            }
            ha_core::filesystem::GitBranchKind::Remote => {
                let short = selected
                    .name
                    .split_once('/')
                    .map(|(_, name)| name)
                ...
                None => {
                    let worktree = db
                        .create_managed_worktree(ha_core::worktree::CreateManagedWorktreeInput {
                            ...
                            purpose: ha_core::worktree::ManagedWorktreePurpose::Manual,
        ...
        if let Some(worktree_id) = created_worktree_id {
            ...
            let cleanup = db
                .run(move |db| -> Result<()> {
                    ...
                    let clean = ha_core::filesystem::git_info(path)
...
fn transfer_workspace(
    ...
    let run_dir = ha_core::paths::git_operation_run_dir(request_id)?;
    if run_dir.exists() {
    ...
    ha_core::platform::write_atomic(&run_dir.join("staged.patch"), &staged)?;
    ha_core::platform::write_atomic(&run_dir.join("unstaged.patch"), &unstaged)?;
...
fn build_change(
    ...
    Ok(Some(GitFileChange {
        ...
        language: ha_core::tools::diff_util::detect_language(&spec.path),
        truncated,
        binary,
        submodule,
        conflicted,
...
fn acquire_repo_lock(repository_identity: &Path) -> Result<RepoLock> {
    let path = ha_core::paths::git_repo_lock_path(repository_identity)?;
    let file = ha_core::platform::try_acquire_exclusive_lock(&path)?
...
fn ensure_session_idle(db: &SessionDB, session_id: &str) -> Result<()> {
    ...
    if !ha_core::async_jobs::JobManager::list_active_work_by_session(session_id)?.is_empty() {
...
fn is_session_busy(session_id: &str) -> bool {
    ha_core::subagent::ACTIVE_CHAT_SESSIONS
        .lock()
...
fn persist_handoff_manifest(run_dir: &Path, manifest: &HandoffManifest) -> Result<()> {
    ...
    ha_core::platform::write_atomic(&run_dir.join("untracked.manifest"), &bytes)?;
    Ok(())
}
    ...
    F: FnOnce(RepoContext) -> Result<GitMutationResult>,
{
    ...
    match run(ctx) {
        Ok(result) => {
            db.complete_git_operation_run(
                request_id,
                result.head.as_deref(),
...
/// Primary 启动期对账（经 `ha_core::vcs_hooks` 的 `git_ops_reconciler`
/// 回调，时序契约见该模块 doc）。handoff 依据落盘 manifest 回滚源
/// checkout；其余操作只标 interrupted（commit/push/PR 结果无法安全重放）。
pub fn reconcile_interrupted_git_operations(db: &SessionDB) -> Result<usize> {
    let stale = db.list_running_git_operations()?;
    for (id, session_id, operation, stage) in &stale {
        let mut recovery_error = None;
        if operation == "handoff" {
            let run_dir = ha_core::paths::git_operation_run_dir(id)?;
            ...
            if manifest_path.is_file() && stage != "snapshotting_source" {
                let recovery = (|| -> Result<()> {
                    ...
                    db.update_session_working_dir(session_id, Some(manifest.source_path.clone()))?;
                    Ok(())
                })();
        ...
        db.mark_git_operation_interrupted(id, &message)?;
    }
    Ok(stale.len())
}
...
fn validate_request_id(id: &str) -> Result<()> {
    let _ = ha_core::paths::git_operation_run_dir(id)?;
    Ok(())
}
